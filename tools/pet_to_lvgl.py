...
def write_atom_header(
    ...
    lines = [
        ...
        "",
        f"static const uint16_t {symbol}_frame_ms[{macro}_FRAMES] = {{260, 260, 260, 260, 260, 360}};",
...
def main() -> None:
    ...
    parser.add_argument("--state", choices=sorted(ROW_SPECS), default="idle")
    ...
    if args.format == "atom":
        boxes = [frame_bbox(rgba, width, row, column) for column in range(count)]
        visible = [box for box in boxes if box is not None]
        if not visible:
            raise SystemExit("selected row has no visible pixels")
        ...
        frames = [
            atom_frame(rgba, width, row, column, crop, args.target_w, args.target_h, args.pad)
