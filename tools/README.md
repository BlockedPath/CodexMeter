...
```bash
node scrape_codexpix.js
```

Fetches the manifest from `codexpix.vercel.app/app.js`, then each animation's
...
Reads `tools/codexpix_data/*.json` and emits a single
