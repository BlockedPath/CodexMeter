...
//   BTN_BACK   (GPIO 0)  — left,  send Space (Codex voice mode push-to-talk)
//   BTN_FWD    (GPIO 18) — right, send Shift+Tab (Codex mode toggle)
...
static uint16_t* alloc_draw_buf(size_t bytes, const char* name) {
    uint16_t* p = (uint16_t*)heap_caps_malloc(bytes, MALLOC_CAP_SPIRAM);
    if (p) {
        Serial.printf("%s allocated in PSRAM (%u bytes)\n", name, (unsigned)bytes);
        return p;
    }

    p = (uint16_t*)heap_caps_malloc(bytes, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    if (p) {
        Serial.printf("%s allocated in internal RAM (%u bytes)\n", name, (unsigned)bytes);
        return p;
    }

    Serial.printf("%s allocation failed (%u bytes)\n", name, (unsigned)bytes);
    return nullptr;
}
...
void setup() {
    ...
    const size_t draw_buf_bytes = LCD_WIDTH * BUF_LINES * 2;
    buf1 = alloc_draw_buf(draw_buf_bytes, "buf1");
    buf2 = alloc_draw_buf(draw_buf_bytes, "buf2");
    // rot_buf needs to hold the largest possible strip after rotation
    // A 480×40 strip rotated 90° becomes 40×480, same pixel count
    rot_buf = alloc_draw_buf(draw_buf_bytes, "rot_buf");

    if (!buf1 || !buf2 || !rot_buf) {
        Serial.println("Display buffer allocation failed; starting BLE without dashboard UI.");
        ble_init();
        return;
    }
...
void loop() {
    ...
    //   RIGHT (GPIO 18) → Shift+Tab (Codex mode toggle)
