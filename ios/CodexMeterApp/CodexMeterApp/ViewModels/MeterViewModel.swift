...
final class MeterViewModel: ObservableObject {
    ...
    private var mdnsHelper: MDNSHelper?
    ...
    func start() {
        ...
        if serverURL.isEmpty {
            mdnsHelper = MDNSHelper(owner: self)
            mdnsHelper?.startBrowsing()
        }
...
private final class MDNSHelper: NSObject {
    weak var owner: MeterViewModel?
    private var browser: NetServiceBrowser?
    private var services: [NetService] = []

    init(owner: MeterViewModel) {
        self.owner = owner
        super.init()
    }
    ...
    private func notify(url: String, name: String) {
        DispatchQueue.main.async { [weak self] in
            guard let owner = self?.owner else { return }
            // reuse existing VM handler
            if !owner.discoveredServers.contains(url) {
                owner.discoveredServers.append(url)
            }
            if owner.serverURL.isEmpty {
                owner.serverURL = url
            }
        }
    }

    private static func firstAddressString(from addrs: [Data]?) -> String? {
        guard let addrs = addrs else { return nil }
        for d in addrs {
            let str = d.withUnsafeBytes { (ptr: UnsafeRawBufferPointer) -> String? in
                guard let base = ptr.bindMemory(to: sockaddr.self).baseAddress else { return nil }
                var host = [CChar](repeating: 0, count: Int(NI_MAXHOST))
                let r = getnameinfo(base, socklen_t(ptr.count), &host, socklen_t(host.count), nil, 0, NI_NUMERICHOST)
                if r == 0 { return String(cString: host) }
                return nil
            }
            if let s = str, !s.isEmpty { return s }
        }
        return nil
    }
}

extension MDNSHelper: NetServiceBrowserDelegate {
...
extension MDNSHelper: NetServiceDelegate {
    func netServiceDidResolveAddress(_ sender: NetService) {
        defer { sender.stop() }
        let name = sender.name
        if let hostName = sender.hostName, !hostName.isEmpty {
            let url = "http://\(hostName):\(sender.port)"
            notify(url: url, name: name)
            return
        }
        if let addr = MDNSHelper.firstAddressString(from: sender.addresses) {
